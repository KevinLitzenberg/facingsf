...
<div id="divMainContent">
	...
	<?php
					...
					if($_SESSION['City'] == "Paris, France"){
						$result = mysql_query("SELECT * FROM faceimages WHERE `ImageLocation` = '". $_SESSION['City']. "' && `Notes`!='SIGNS and NOTES' ORDER BY `ImageDate` DESC");
					}
					...
					//Check for Southeast Asia
						...
						$result= mysql_query("SELECT * FROM faceimages WHERE `ImageLocation` LIKE '%Thailand' && `Notes`!='SIGNS and NOTES' OR `ImageLocation` LIKE '%Malaysia' && `Notes`!='SIGNS and NOTES' OR `ImageLocation` LIKE 'Singapore, Singapore'&& `Notes`!='SIGNS and NOTES' ORDER BY `ImageDate` DESC");
					...
					//Check for Central America
						...
						$result= mysql_query("SELECT * FROM faceimages WHERE `ImageLocation` LIKE '%Nicaragua' && `Notes`!='SIGNS and NOTES'  OR `ImageLocation` LIKE '%Mexico' && `Notes`!='SIGNS and NOTES' ORDER BY `ImageDate` DESC");

                    }	

					//Check for Signs and Notes
					else if($_SESSION['City'] == "Signs and Notes"){
                        $result= mysql_query("SELECT * FROM faceimages WHERE `Notes` = 'SIGNS and NOTES' ORDER BY `ImageDate` DESC");
                    }
					    ...
					    $result = mysql_query("SELECT * FROM faceimages WHERE `ImageLocation` = '". $_SESSION['City']. "' && YEAR(`ImageDate`) = '". $_SESSION['Year']."' && `Notes`!='SIGNS and NOTES' OR `ImageLocation`= 'Oakland, CA' && YEAR(`ImageDate`) = '". $_SESSION['Year']."'&& `Notes`!='SIGNS and NOTES' ORDER BY `ImageDate` DESC");
